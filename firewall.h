    ...
    static void analyzePacket(const u_char* packet, const struct pcap_pkthdr* header) {
        ...
        if (ntohs(eth->ether_type) == ETHERTYPE_IP) {
            ...
            if(iph->ip_p == 6){
                ...
                if((flags & TH_SYN) && !(flags & TH_ACK)){//SYN флуд
                    checkFloodAttack(src_ip,syn_count_map,1,"SYN flood");
                }
                ...
                else if((flags & TH_FIN) && (flags & TH_URG) && (flags & TH_PUSH)) {// Xmas Scan
...
private:
   static void checkFloodAttack(uint32_t src_ip,
    ...
    static inline std::vector<uint32_t> SYNatack_ip_pool = {};
};
